...
export class SupersetService {
  ...
  constructor() {
    ...
    this.axiosInstance.interceptors.response.use((resp) => {
      if (resp.config.url?.includes("/api/v1/security/csrf_token/")) {
        this.logger.log("[/csrf_token/] set-cookie:", resp.headers["set-cookie"]);
      }
      return resp;
    });
    this.axiosInstance.interceptors.request.use(async (config) => {
      ...
      if (config.url?.includes("/api/v1/security/guest_token/")) {
        this.logger.log(`[req ${config.method} ${config.url}] Cookie: ${cookieStr}`);
      }
      return config;
    });
  }
  ...
  private async getCsrfToken(): Promise<string> {
    try {
      ...
      if (!token) {
        throw new HttpException("CSRF token missing in response", HttpStatus.BAD_GATEWAY);
      }
  
      // --- Capture Set-Cookie header(s) from Superset
      const setCookieHdr = (resp.headers as any)["set-cookie"] as string[] | undefined;
      ...
      if (!finalHasSession || !finalHasCsrf) {
        this.logger.error("[SupersetService] cookie check failed after /csrf_token/", {
          cookies: cookies.map(c => `${c.key}; Domain=${c.domain}; Path=${c.path}; Secure=${c.secure}`)
        });
        throw new HttpException("CSRF/session cookie missing", HttpStatus.BAD_GATEWAY);
      }
  ...
  private async authenticate(): Promise<string> {
    const now = Date.now();
    if (this.accessToken && now < this.accessExp - 30_000) return this.accessToken;
  ...
  async generateGuestToken(
    ...
    this.logger.log(
      ...
      cookies.map(c => `${c.key}=${c.value}; Domain=${c.domain}; Path=${c.path}; Secure=${c.secure}; SameSite=${c.sameSite}`)
    );
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        throw new HttpException(
          err.response?.data?.message || JSON.stringify(err.response?.data) || "guest_token failed",
